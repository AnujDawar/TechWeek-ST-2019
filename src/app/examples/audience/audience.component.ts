...
export class AudienceComponent implements OnInit {
	...
	submit(form: NgForm) {
		...
		if (this.choice_decr == this._data[0].choice_1)
			selectedChoice = "A";

		if (this.choice_decr == this._data[0].choice_2)
			selectedChoice = "B";

		if (this.choice_decr == this._data[0].choice_3)
			selectedChoice = "C";

		if (this.choice_decr == this._data[0].choice_4)
			selectedChoice = "D";
	...
	refreshData(): any {
		this.postsSubscription = this.restApi
			.get(
				aws_url.GET_CURRENT_QUESTION_URL +
				"?email=" +
				this.globalService.localStorageItem("email")
			)
			.subscribe(
				data => {
					this._data = data;
